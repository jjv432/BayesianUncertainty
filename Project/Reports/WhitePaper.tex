...
The model that will be employed for this analysis is an 'S'-shaped structure with discrete components. These components are shown below in Figure \ref{fig:SpringAnnotated}. The red 'connecters' are assumed to be rigid bodies- they will remain at the same shape regardless of applied loads. The green 'elbows' are the compliant components of this system. A derivation of how they react to stress is discussed below. It is assumed that the applied force is equally distributed to both elbows, and that both elbows deform equally. Furthermore, it is assumed that the bottom 'connector' is rigidly attached to the ground. Finally, it is assumed that all components are mass-less, with all of the mass residing at a point on the top and center of the top 'connector.'


The first four parameters of this system describe the shape of the spring. Most of these parameters are displayed in Figure \ref{fig:SpringAnnotated}. A parameter not shown in \ref{fig:SpringAnnotated} is the Width 'w,' which is the dimension into the page. This is a constant value for all of the components of the spring.
...
The set of parameters $\theta$ that will be considered random variables for future uncertainty quantification are the parameters that define the spring as well as a ratio $\alpha$ that defines the damping coefficient:  
...
\subsubsection{Characterizing Elbow Response}
When a force is applied to one of the connectors, it creates a moment about the elbows. To simplify calculations, this moment is considered to only deform the elbow by means of bending stress. Furthermore, it is assumed that the radius, thickness, and the arc-length of the inner face remain constant. This means that all of the strain that the elbows encounter will be applied to the arc-length of the outer edge only. This strain is assumed to only travel in one direction: `upward', or toward the connector above the elbow.  The result of this is an elbow whose only degree of freedom is the angle that it's top connector interface makes with the horizontal plane.
...
\begin{equation}
	\theta_{opt} = \begin{bmatrix}
	0.0497  &  0.0007  &  2.97e-07 &  0.0581 & 0.0100
\end{bmatrix}^T
\label{eq:ThOpt}
\end{equation}
