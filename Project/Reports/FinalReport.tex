...
\begin{figure}[H]
 ...
 From Figure \ref{fig:LocalSens}, it can be concluded that $\theta_5$ is insensitive and therefore is unidentifiable. For this reason, the parameter set, defined in Equation \ref{eqn:OGParms}, can be reduced to:
...
The response of each parameter was plotted against the response of every other parameter, as shown in Figure \ref{FullPairs}. A number of dependencies between parameters are made apparent by this figure, namely those between $w$ and $L$, and $\alpha$ and $L$ and $w$.
...
% Reduced set
 ...
 The parameter set $\theta_{RED}$ is further reduced to account for the results in the previous section. Herein, $\theta_{RED}$ is defined by Equation \ref{eqn:ThetaRedRed}.
 ...
 The results of the MCMC for the reduced parameter set is shown below. Figure \ref{fig:RedChains} shows that these parameters have begun to converge to a 
