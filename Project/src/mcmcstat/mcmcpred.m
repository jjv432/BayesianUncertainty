...
for i=1:nbatch
  ...
  for iisample = 1:nsample;
    theta(parind) = chain(isample(iisample),:)'; %sample out of posteriors
    th  = theta(local==0|local==i);
    y   = feval(modelfun,datai,th,varargin{:}); %evaluate model with theta samples
    ysave(iisample,:,:) = y;
    if ~isempty(s2chain)
      if sstype==0
        osave(iisample,:,:) = ...
  ...
  [my,ny] = size(y);  %outputs results for a given set of standard deviations
  for j=1:ny %create the prediction interval limits
    if 0& nbatch == 1 & ny == 1
    ...
    if ~isempty(s2chain) %create the credible interval limits
...
% expand ydata to include enough time values
time = y(:,1);
